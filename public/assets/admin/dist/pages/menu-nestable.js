...
var MenuNestable = function () {
    ...
    function set_data_items(target)
    {
        target.each(function(index, el) {
            ...
            current.data('postid', current.attr('data-postid'));
            current.data('catid', current.attr('data-catid'));
    ...
    return {
        ...
        handleNestableMenu: function()
        {
            ...
            $body.on('click', '.box-links-for-menu .btn-add-to-menu', function(event) {
                event.preventDefault();
                var current = $(this);
                var parent = current.parents('.tab-pane');
                var html_src = '';
                ...
                if(parent.attr('id') == 'external_link')
                {
                    ...
                    html_src += '<li data-type="'+data_type+'" data-catid="'+data_catid+'" data-postid="'+data_postid+'" data-title="'+data_title+'" data-class="'+data_css_class+'" data-id="0" data-customurl="'+data_url+'" data-iconfont="" class="dd-item dd3-item">';
                ...
                else
                {
                    parent.find('.list-item > li.active').each(function(index, el) {
                        ...
                        var data_catid = find_in.attr('data-catid');
                        var data_postid = find_in.attr('data-postid');
                        ...
                        html_src += '<li data-type="'+data_type+'" data-catid="'+data_catid+'" data-postid="'+data_postid+'" data-title="" data-class="" data-id="0" data-customurl="" data-iconfont="" class="dd-item dd3-item">';
                        html_src += '<div class="dd-handle dd3-handle"></div>';
                        html_src += '<div class="dd3-content">';
                        html_src += '<span class="text pull-left" data-update="title">'+data_title+'</span>';
                        html_src += '<span class="text pull-right">'+data_type+'</span>';
            ...
            // Submit
            $body.on('click', '.form-save-menu [type="submit"]', function(event) {
                event.preventDefault();
                var current = $(this);
